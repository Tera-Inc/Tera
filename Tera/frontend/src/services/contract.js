 ...
 * loaded from the VITE_TERA_WASM_HASH environment variable or
...
export async function deployContract(walletId) {
  ...
  if (!wasmHash) {
    try {
      ...
      throw new Error(
        'Soroban WASM hash not configured. Set VITE_TERA_WASM_HASH or ensure the backend serves /api/contract-wasm-hash.'
      );
    }
  }
  if (!wasmHash) {
